...
if [ "$ENABLE_WARP" = "true" ]; then
    ...
    if [ $COUNT -lt $MAX_RETRIES ]; then
        ...
        (warp-cli --accept-tos tunnel host add www.lokke.app > /dev/null 2>&1 || \
         warp-cli --accept-tos add-excluded-domain www.lokke.app > /dev/null 2>&1) || true

        (warp-cli --accept-tos tunnel host add mediahubmx.cc > /dev/null 2>&1 || \
         warp-cli --accept-tos add-excluded-domain mediahubmx.cc > /dev/null 2>&1) || true
